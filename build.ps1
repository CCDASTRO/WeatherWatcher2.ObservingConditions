...
& $msbuild (Join-Path $root 'WeatherWatcher2.ObservingConditions\WeatherWatcher2.ObservingConditions.csproj') /p:Configuration=Release /v:minimal /nologo
if ($LASTEXITCODE) { throw 'WeatherWatcher build failed' }
& $msbuild (Join-Path $root 'tests\AmbientTests.csproj') /p:Configuration=Release /v:minimal /nologo
