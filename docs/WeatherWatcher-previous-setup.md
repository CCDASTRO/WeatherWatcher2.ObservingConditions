...
## RainCloud development integration (1.1.0)
Optional Uno RG-11/MLX90614 input adds calibrated sky/cloud readings and fail-closed combined safety. Requires matching SafetyMonitor 1.3.0 for expiring safety records. Disabled by default; not hardware-qualified. See [integration guide](docs/RainCloud.md). 126 offline regression checks pass.

### Without a RainCloud sensor
Leave Use Uno RainCloud disabled (the default). No sensor COM port is opened, and the existing weather configuration remains in effect. Enabling it makes fresh, valid and calibrated sensor data a requirement for safety; there is no automatic fallback to safe when hardware is missing.

### Combined installer
The development Inno suite installs WeatherWatcher 1.1.0 and SafetyMonitor 1.3.0 together. ASCOM profiles are retained. The suite has compiled successfully but has not been installed or hardware-qualified. See [HTML usage guide](docs/weatherwatcher-guide.html) and [RainCloud integration](docs/RainCloud.md).
